...
public class CompanyUserService {
    ...
    private final IsUserExists isUserExists;
    private final IsAdminOrCompanyAdmin isAdminOrCompanyAdmin;
    private final HasPermission hasPermission;

    public void link(User connectedUser, long companyId, long userId, LinkCompanyUser.Role role) throws ForbiddenActionException, ResourceNotFoundException {
        isCompanyExists.execute(companyId);
        isUserExists.execute(userId);
        hasPermission.execute(connectedUser, isCompanyAdmin(connectedUser, companyId), role);

        companyUserProvider.link(new LinkCompanyUser(companyId, userId, role));
    }
    ...
    public Optional<LinkCompanyUser> find(User connectedUser, long companyId, long userId) throws ForbiddenActionException, ResourceNotFoundException {
        isCompanyExists.execute(companyId);
        isUserExists.execute(userId);
        isAdminOrCompanyAdmin.execute(connectedUser, companyId);
    ...
    public void unlink(User connectedUser, long companyId, long userId) throws ForbiddenActionException {
        ...
        if(companyUser.isPresent()) {
            hasPermission.execute(connectedUser, isCompanyAdmin(connectedUser, companyId), companyUser.get().getRole());
            companyUserProvider.unlink(companyUser.get());
        }
    }

    public void unlinkAllByCompanyId(User connectedUser, long companyId) throws ForbiddenActionException, ResourceNotFoundException {
        isCompanyExists.execute(companyId);

        if(connectedUser.isAdmin()) {
            companyUserProvider.unlinkAllByCompanyId(companyId);
        }

        if(isCompanyAdmin(connectedUser, companyId)) {
            companyUserProvider.unlinkAllByCompanyIdExceptAdminRole(companyId);
        }
    }

    public void unlinkAllByUserId(User connectedUser, long userId) throws ResourceNotFoundException, ForbiddenActionException {
        isAdminUser.execute(connectedUser);
        isUserExists.execute(userId);

        companyUserProvider.unlinkAllByUserId(userId);
    }
