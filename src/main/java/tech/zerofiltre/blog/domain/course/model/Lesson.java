...
@JsonIgnoreProperties(value = {"courseProvider", "userProvider", "chapterProvider", "lessonProvider", "enrollmentProvider", "checker"})
public class Lesson {
    ...
    private Lesson(LessonBuilder lessonBuilder) {
        ...
        this.checker = lessonBuilder.checker;
        this.number = lessonBuilder.number;
        this.notEnrolledAccess = lessonBuilder.notEnrolledAccess;
    }
    ...
    void checkLessonAccessConditions(long currentUserId, long chapterId, boolean isDeletion, boolean checkEnrollments) throws ResourceNotFoundException, ForbiddenActionException {
        ...
        checkConditionsForUserForUnpublishedCourse();
        checkConditionsForDeletingLesson(currentUserId, isDeletion);
        checkConditionsForUserWithoutConsideringEnrollments(checkEnrollments);
        checkConditionsWithConsideringEnrollments(currentUserId, courseId, checkEnrollments);
    }

    private void checkConditionsForUserForUnpublishedCourse() throws ForbiddenActionException {
    ...
    private void checkConditionsForDeletingLesson(long currentUserId, boolean isDeletion) throws ForbiddenActionException {
        if (isDeletion
                && course.getStatus().equals(Status.PUBLISHED)
                && !currentUser.isAdmin()) {
    ...
    private void checkConditionsForUserWithoutConsideringEnrollments(boolean checkEnrollments) throws ForbiddenActionException {
        if (!checkEnrollments) {
            checkUserAuthorized(currentUser, course, "You are not allowed to do this action on this course");
        }
    }

    private void checkConditionsWithConsideringEnrollments(long currentUserId, long courseId, boolean checkEnrollments) throws ForbiddenActionException {
        if (checkEnrollments) {
            Optional<Enrollment> enrollment = enrollmentProvider.enrollmentOf(currentUserId, courseId, true);
            if (enrollment.isEmpty() && !free) {
                Optional<Long> companyId = courseProvider.idOfCompanyOwningCourse(course.getId());
    ...
    public static class LessonBuilder {
        ...
        public LessonBuilder checker(DataChecker checker) {
