...
public class StackOverflowAuthenticationFilter extends OncePerRequestFilter {
    ...
    public StackOverflowAuthenticationFilter(StackOverflowTokenConfiguration stackOverflowTokenConfiguration, StackOverflowProvider stackOverflowProvider, UserProvider userProvider) {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain chain)
        ...
        try {    // exceptions might be thrown in validating the token: if for example the token is expired
            ...
            if (stackOverflowProvider.isValid(token)) {
                //5. Get the user info from the token
                stackOverflowProvider.userOfToken(token)
                        .ifPresent(user -> {
