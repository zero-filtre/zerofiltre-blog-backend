...
public class DBCompanyCourseProvider implements CompanyCourseProvider {
    ...
    @Override
    public LinkCompanyCourse link(LinkCompanyCourse linkCompanyCourse) {
        return mapper.fromJPA(repository.save(mapper.toJPA(linkCompanyCourse)));
    }

    @Override
    public Optional<LinkCompanyCourse> linkOf(long companyId, long courseId) {
        return repository.findByCompanyIdAndCourseId(companyId, courseId).map(mapper::fromJPA);
    }

    @Override
    public Optional<LinkCompanyCourse> linkOf(long companyId, long courseId, boolean active) {
    ...
    public Page<LinkCompanyCourse> findAllByCompanyIdByPage(int pageNumber, int pageSize, long companyId) {
        org.springframework.data.domain.Page<CompanyCourseJPA> pageJpa = repository.findAllByCompanyId(PageRequest.of(pageNumber, pageSize), companyId);
    ...
    @Override
    public void unlink(LinkCompanyCourse linkCompanyCourse) {
        repository.delete(mapper.toJPA(linkCompanyCourse));
    }

    @Override
    public void unlinkAllByCompanyId(long companyId) {
        repository.deleteAllByCompanyId(companyId);
    }

    @Override
    public void unlinkAllByCourseId(long courseId) {
