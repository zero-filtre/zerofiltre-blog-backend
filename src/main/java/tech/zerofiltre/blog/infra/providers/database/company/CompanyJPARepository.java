...
public interface CompanyJPARepository extends JpaRepository<CompanyJPA, Long> {
    ...
    @Query("SELECT c FROM CompanyJPA c JOIN LinkCompanyUserJPA l ON c.id = l.companyId AND l.userId = ?1 AND l.active = true ")
    Page<CompanyJPA> findAllByUserId(Pageable pageable, long userId);

    @Query("SELECT c.id FROM CompanyJPA c " +
            "JOIN LinkCompanyUserJPA cu ON c.id = cu.companyId and cu.active = true and cu.userId = ?1 " +
            "JOIN LinkCompanyCourseJPA cc ON c.id = cc.companyId and cc.active = true and cc.courseId = ?2")
