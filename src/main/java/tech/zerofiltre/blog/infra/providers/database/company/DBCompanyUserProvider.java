...
public class DBCompanyUserProvider implements CompanyUserProvider {
    ...
    @Override
    public LinkCompanyUser link(LinkCompanyUser linkCompanyUser) {
    ...
    public Page<LinkCompanyUser> findAllByCompanyId(int pageNumber, int pageSize, long companyId) {
        org.springframework.data.domain.Page<CompanyUserJPA> pageJpa = repository.findAllByCompanyId(PageRequest.of(pageNumber, pageSize), companyId);
        return pageMapper.fromSpringPage(pageJpa.map(mapper::fromJPA));
    }

    @Override
    public void unlink(LinkCompanyUser linkCompanyUser) {
    ...
    @Override
    public void unlinkAllByCompanyIdExceptAdminRole(long companyId) {
