...
class DBCompanyUserProviderIT {
    ...
    void shouldReturnList_whenSearchingUserCompaniesAndRoles() {
        ...
        //WHEN
        List<UserCompanyInfosVM> response = dbCompanyUserProvider.findCompaniesAndRolesByUserId(user.getId());

        //THEN
        assertThat(response).isNotNull().hasSize(3);
        assertThat(response.get(0).getCompanyId()).isEqualTo(company1.getId());
        assertThat(response.get(0).getUserRole()).isEqualTo(LinkCompanyUser.Role.ADMIN.toString());
        assertThat(response.get(1).getCompanyId()).isEqualTo(company2.getId());
        assertThat(response.get(1).getUserRole()).isEqualTo(LinkCompanyUser.Role.EDITOR.toString());
        assertThat(response.get(2).getCompanyId()).isEqualTo(company3.getId());
        assertThat(response.get(2).getUserRole()).isEqualTo(LinkCompanyUser.Role.VIEWER.toString());
    }
