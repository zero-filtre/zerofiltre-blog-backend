...
class CompanyUserServiceTest {
    ...
    @Test
    @DisplayName("when findAllUsersByCompanyId then verify call companyUserProvider findAllUsersByCompanyId")
    void whenFindAllUsersByCompanyId_thenVerifyCallCompanyUserProviderFindAllByCompanyId() throws ResourceNotFoundException, ForbiddenActionException {
        //GIVEN
        when(checker.companyExists(anyLong())).thenReturn(true);
        when(checker.isAdminOrCompanyAdmin(any(User.class), anyLong())).thenReturn(true);
